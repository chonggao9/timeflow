  ...
  try {
    ...
    const release = await gh(`https://api.github.com/repos/${REPO}/releases`, {
      ...
      body: JSON.stringify({
        tag_name: `v${next}`,
        name: `v${next}`,
        body: `TimeFlow v${next}\n\n### 🚀 更新与修复内容\n- **稳定性紧急修复**：修复时间轴组件缺少样式声明导致的冷启动空白崩溃缺陷；\n- **全局错误边界保护**：引入根层级 ErrorBoundary，杜绝子组件渲染异常直接引起整屏白屏；\n- **发版静态代码门禁**：集成基于 AST 的全库自动化静态审查与未声明变量校验；\n- **打卡交互重构**：全宽居中主按钮，轻按记录途经点，长按（>500ms）一步记录终点并完结行程；\n- **智能超时兜底**：行程断开阈值优化为 90 分钟，自适应停留切分新行程；\n- **9 大出行方式扩展**：新增打车、地铁、高铁、飞机、轮渡，覆盖日常通勤与城际差旅全场景；\n- **水平滚动胶囊栏**：平滑手感、自动记忆居中聚焦，时间轴连线上直观呈现出行方式与耗时；\n- **隐私协议更新**：如实同步 9 种出行方式枚举与 WebDAV 自主云同步披露。\n\n安装：下载下方 APK 即可直接安装。`,
        draft: false,
        prerelease: false,
      }),
    });
